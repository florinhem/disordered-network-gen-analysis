...
function fit_and_predict(df::DataFrames.DataFrame, T_max_interp, beta_interp, theta_interp)
    ...
    output_data = df[:, [:SigmaLen, :SigmaAng]]
...
function get_data_and_predict(;
    ...
    DataFrames.rename!(df, 
        ...
        .=>  [:NrVertices, :MaxT, :Beta, :GradT, :MCsteps, :Theta, :Trial, :SigmaLen, 
            :SigmaAng, :AccMoves, :StretchEnergy, :BendEnergy ,
    ...
    df_filtered[:, :SigmaLenPred] = fill(NaN, DataFrames.nrow(df_filtered))
    df_filtered[:, :SigmaAngPred] = fill(NaN, DataFrames.nrow(df_filtered))
    ...
    for (i, (T_max_interp, beta_interp, theta_interp)) in enumerate(zip(df_filtered[:, :MaxT], df_filtered[:, :Beta], df_filtered[:, :Theta]))
        ...
        df_filtered[i, :SigmaLenPred] = bond_length_std_interp
        df_filtered[i, :SigmaAngPred] = bond_angle_std_interp
    end

    df_filtered[:, :SigmaLenRatio] = df_filtered[:, :SigmaLenPred] ./ df_filtered[:, :SigmaLen]
    df_filtered[:, :SigmaAngRatio] = df_filtered[:, :SigmaAngPred] ./ df_filtered[:, :SigmaAng]
