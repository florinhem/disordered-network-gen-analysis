...
"""
Get 2 random coordinates that lie inside the structure at a given direction 
to each other
"""
function get_random_coordinates_at_direction(
    sampling_vec::Vector,
    size_data::Tuple)
...
function get_autocovariance_fct(
    ...
    two_point_prob_fct = Measurements.measurement( 
        ...
        sqrt( structure_dict["volume_fract_tot"]^2 
            * (1 - structure_dict["volume_fract_tot"]^2) 
            / nr_measurements_per_direction  ) )

    # determine autocovariance function
    autocovariance_fct = two_point_prob_fct - 
        structure_dict["volume_fract_tot"]^2
...
function get_autocovariance_fct_by_sampling_vec_array(
    ...
    for i in 1:autocovariance_fct_array_size[1]
        ...
        println("Autocovariance calculation: layer "*string(i)
            *" along x direction done" )
...
^
    ...
    autocovariance_fct_direction_dict = modify_keys_in_dict(
        autocovariance_fct_direction_dict, voxel_edge_length, label)
...
When autocovariance function is calculated as a function of sampling vectors, 
only half of the 3d space with with z>=0 is considered. 
Due to the mirror symmetry of the autocovariance function, 
this contains the full information. I think for the Fast Fourier Transform 
the full data is needed however
"""
function get_complete_autocovariance_fct_by_sampling_vec_array(
    ...
    # only positive z values where considered
    ...
    complete_sampling_vec_array = get_vector_array(
        complete_sampling_distance_vec_vec)

    # create an autocovariance function array that is mirrored in the 
    # first two dimensions
    autocovariance_fct_array_mirrored = 
        reverse( reverse( reverse(
            autocovariance_fct_direction_dict["autocovariance_fct_array"], 
        dims=1), dims=2), dims=3) 

    # get mirrored autocovariance function
    complete_autocovariance_fct_array = cat(
        autocovariance_fct_array_mirrored[:,:,1:end-1],
        autocovariance_fct_direction_dict["autocovariance_fct_array"],
        dims = 3)
    ...
    complete_autocovariance_fct_direction_dict = Dict(
        "sampling_vec_array" => complete_sampling_vec_array,
        ...
        "nr_measurements_per_direction" 
                => autocovariance_fct_direction_dict[
                    "nr_measurements_per_direction"],
        "voxel_edge_length" => autocovariance_fct_direction_dict[
            "voxel_edge_length"],
...
"""
if the autocovariance function was measured as a function of 
all possible directions for a single unit cell of a perfectly periodic medium 
(e.g. one created from a nodal equation),
extrapolate the autocovariance function of many unit cells 
(50x50x50 as a standard).
The dictionary that is passed as an argument, should contain the 
following keys:
- sampling_vec_array
- sampling_distance_vec_vec
...
function extrapolate_periodic_data_autocovariance_fct_by_sampling_vec_array(
    ...
    extrapolated_sampling_distance_vec_vec = get_sampling_distance_vec_vec(
        size_data_single_unit_cell 
        .* nr_unit_cells)
    
    # get the corresponding array of vectors, where along the fourth dimension 
    # the three vector components are stored
    extrapolated_sampling_vec_array = get_vector_array(
        extrapolated_sampling_distance_vec_vec)

    
    # get size of extrapolated autocovariance fct array
    extrapolated_autocovariance_fct_array_size = size(
        extrapolated_sampling_vec_array)[1:3]

    # initialize the extrapolated autocovariance fct array
    extrapolated_autocovariance_fct_array = Array{Measurements.Measurement}(
        undef, 
        extrapolated_autocovariance_fct_array_size...)

    
    # determine the maximal sampling distances inside the single unit cell 
    ...
    for i in extrapolated_sampling_distance_vec_vec[1]
        for j in extrapolated_sampling_distance_vec_vec[2]
            for k in extrapolated_sampling_distance_vec_vec[3]
                ...
                vector_within_unit_cell = ( mod.( [i,j,k] 
                    .+ max_sampling_distances, 
                    size_data_single_unit_cell) 
                    .- max_sampling_distances )
                
                # if the z component of that vector is negative, 
                # it needs to be flipped, because due to its mirror symmetry, 
                # the autocovariance function was only calculated for 
                # positive z components
                if vector_within_unit_cell[3] < 0
                    vector_within_unit_cell .*= (-1)
                end

                # find the entries of this vector in the 
                # sampling_distance_vec_vec of the single unit cell
                current_indices_within_unit_cell = findfirst.( .==( 
                    vector_within_unit_cell ), 
                    autocovariance_fct_direction_dict[
                        "sampling_distance_vec_vec"]  )

                # save the corresponding autocovariance function values 
                # to the array of the extrapolated autocovariance function
                extrapolated_autocovariance_fct_array[
                    i + extrapolated_sampling_distance_vec_vec[1][end] + 1,
                    j + extrapolated_sampling_distance_vec_vec[2][end] + 1,
                    k + 1] = (
                    autocovariance_fct_direction_dict[
                        "autocovariance_fct_array"][
                            current_indices_within_unit_cell...]
                    )
    ...
    # create dict to save
    extrapolated_autocovariance_fct_direction_dict = Dict(
        "sampling_vec_array" => extrapolated_sampling_vec_array,
...
"""
function get_autocovariance_fct_along_direction_vec(
    direction_vec::Vector, 
    autocovariance_fct_array::Array)

    # if z coordinate is negative, mirror the vector, because 
    # autocovariance function was only measured for positive z coordinates
    ...
    # set current position to indices of autocovariance_fct_array where 
    # autocovariance function at sampling vector [0,0,0] was measured
    current_position = [ 
        Int.( (size_autocovariance_fct_array[1:2] .+ 1) ./2 ) ..., 1]
    ...
    while (prod( current_position .<= size_autocovariance_fct_array )
        *prod( current_position .>= [1,1,1] ) == 1)
        ...
        push!(autocovariance_fct_along_direction_vec, 
            autocovariance_fct_array[Int.( round.( current_position ) ) ...] )
...
function get_wavenumber_vec_vec(autocovariance_fct_array::Array)
    ...
    wavenumber_vec_vec = (2*pi) .* [
        ...
        FFTW.fftshift( FFTW.fftfreq( size(autocovariance_fct_array)[3] ) ) ]
...
function get_spectral_density_by_wavevector_array_fft(
    ...
    spectral_density_array_fft_output = FFTW.rfft(
        complete_autocovariance_fct_array_values)

    # bring spectral densities into an order from negative to 
    ...
    spectral_density_dict = Dict(
        ...
        "voxel_edge_length" => complete_autocovariance_fct_direction_dict[
            "voxel_edge_length"],
