...
"""
Apply gaussian filter of given sigma (standard deviation) to 1D data
that is non equidistantly spaced. Calculation is inspired by
https://stackoverflow.com/questions/24143320/gaussian-sum-filter-for-irregular-spaced-points
"""
function gaussian_filter_1d(data_x::Vector{Float64}, data_y::Vector; 
    ...
    for i in eachindex(filtered_data_x)
        ...
        weights = exp.(-0.5 * delta_x[window_lower_index:window_upper_index] .^ 2 / sigma_x^2)

        # normalize the weights
        weights = weights ./ sum(weights)

        # calculate the filtered data point
        filtered_data_y[i] = LinearAlgebra.dot(data_y[window_lower_index:window_upper_index], weights)
