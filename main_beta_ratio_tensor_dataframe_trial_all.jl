...
function get_data_and_predict(;
    ...
    # Create new columns for the interpolated values
    df_filtered[:, :BondAngleStdInterp] = -1
    df_filtered[:, :BondLengthStdInterp] = -1

    # Iterate over unique combinations of T_max, beta, and theta
    for (T_max, beta, theta) in unique(df_filtered[:, [:MaxT, :Beta, :Theta]])
        bond_angle_std_interp, bond_length_std_interp = fit_and_predict(df_filtered, T_max, beta, theta)
        println("Interpolated bond_angle_std for T_max=$T_max, beta=$beta, theta=$theta: $bond_angle_std_interp")
        println("Interpolated bond_length_std for T_max=$T_max, beta=$beta, theta=$theta: $bond_length_std_interp")
        
        # Store the interpolated values in the dataframe
        filter!(row -> row.MaxT == T_max && row.Beta == beta && row.Theta == theta, df_filtered) do row
            row.BondAngleStdInterp = bond_angle_std_interp
            row.BondLengthStdInterp = bond_length_std_interp
        end
    end

    println("df_filtered with interpolated values:")
    display(df_filtered)
...
get_data_and_predict(
    ...
    bond_bending_const_array=[0.2],#[0.2,0.25,0.3,0.35,0.4],
    temperature_gradient_array=[0.1],
    nr_monte_carlo_steps_per_temperature_array=[0.01],
    theta_ground_state_array=[100.0],#[100.0,120.0,160.0,180.0],                 
    nr_trials_per_temperature_array=[1], #[1,2,3,4,5],
    T_max_interp = 0.15,
    beta_interp = 0.3,
    theta_interp = 140,
