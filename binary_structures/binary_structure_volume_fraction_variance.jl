...
"""
function get_window_index_domains(
    nr_dimensions_data::Int64,
    edge_length_window::Int64, 
    random_window_center::Tuple)

    # create empty arrays where upper and lower index bounds of the 
    # measuring window on the data array will be stored
    ...
    for i in 1:nr_dimensions_data
        lower_indices_vector[i] = 
            random_window_center[i] - Int( (edge_length_window - 1)/2  )
        upper_indices_vector[i] = 
            random_window_center[i] + Int( (edge_length_window - 1)/2  )
...
"""
determine index coordinates that lie within a sphere of given center position 
and diameter
"""
function get_coord_inside_window_vec(
        nr_dimensions_data::Int64,
        edge_length_window::Int64, 
        window_center::Tuple
        ;
    ...
    lower_indices_vector, upper_indices_vector = get_window_index_domains(
        nr_dimensions_data,
        edge_length_window,
        window_center)
...
"""
function get_nr_independent_samples(
    nr_dimensions_data::Int64, 
    edge_length_window::Int64, 
...
"""
determine the number of measurements per window size depending on 
positioning and shape
"""
function get_nr_measurements(
    nr_dimensions_data::Int64, 
    size_data::Tuple,
    edge_length_window::Int64
    ; 
    ...
    if cmp(window_positioning, "random") == 0
        ...
        volume_window =  get_window_volume(
            nr_dimensions_data, 
            edge_length_window;
            window_shape=window_shape)
    ...
    elseif cmp(window_positioning, "scanned") == 0
        ...
        if cmp(window_shape, "spherical") == 0
            ...
            nr_measurements = Int( prod( 
                2 .* floor.( size_data ./ (edge_length_window + 1) )  .-1 ) )
...
"""
perform a single measurement on local volume fraction and update the 
sampled voxels array
"""
function get_local_volume_fract_and_update_sampled_voxels!(
    nr_dimensions_data::Int64,
    edge_length_window::Int64,
    ...
    # determine index coordinates that lie within the window depending on 
    # its shape
    coord_inside_window_vec = get_coord_inside_window_vec(
        nr_dimensions_data,
        edge_length_window, 
        window_center;
        window_shape = window_shape)
    ...
    # sum over data entries within window and continuously update 
    # sampled voxels array
...
"""
function get_random_window_center(
    nr_dimensions_data::Int64, 
    size_data::Tuple,
    edge_length_window::Int64)
...
"""
function get_window_centers_vec(
    nr_dimensions_data::Int64, 
    edge_length_window::Int64,
    size_data::Tuple,
    ...
    if cmp(window_positioning, "random") == 0
        ...
        for i in eachindex(window_centers_vec)
            window_centers_vec[i] = get_random_window_center(
                nr_dimensions_data, 
                size_data,
                edge_length_window)
        end
    ...
    elseif cmp(window_positioning, "scanned") == 0
        ...
        if cmp(window_shape, "spherical") == 0
            ...
            # the spherical windows are shifted such that the centers 
            # of neighboring windows do not overlap 
            # therefore +1 is added to the window edge length
            nr_measurements_along_dimensions = 
                Int.( 2 .* floor.( size_data ./ (edge_length_window+1) ) .-1 )
            ...
            for i in 1:nr_measurements_along_dimensions[1]
                for j in 1:nr_measurements_along_dimensions[2]
                    for k in 1:nr_measurements_along_dimensions[3]
                        ...
                        window_centers_vec[current_nr_measurement] = ( 
                            i * Int( (edge_length_window+1)  /2),
        ...
        elseif cmp(window_shape, "cubic") == 0
            ...
            # determine nr of measurements along all dimensions
            nr_measurements_along_dimensions = 
                Int.( floor.( size_data ./ edge_length_window ) )
            ...
            for i in 0:(nr_measurements_along_dimensions[1] - 1 )
                for j in 0:(nr_measurements_along_dimensions[2] - 1 )
                    for k in 0:(nr_measurements_along_dimensions[3] - 1 )
                        ...
                        window_centers_vec[current_nr_measurement] = ( 
                            Int( (1/2 + i) * edge_length_window + 1/2 ),
...
"""
function get_local_volume_fract_vec(
    nr_dimensions_data::Int64, 
    edge_length_window::Int64,
    size_data::Tuple, 
    data_binary::Array{Bool}
    ...
    nr_measurements = get_nr_measurements(
        nr_dimensions_data, size_data,
        edge_length_window; 
    ...
    # get vector of window centers depending on their positioning and shape
    window_centers_vec = get_window_centers_vec(
        nr_dimensions_data, 
        edge_length_window,
        size_data,
        nr_measurements;
    ...
    local_volume_fract_vec = Vector{Float64}(undef, nr_measurements)  
        # zeros(Float64, nr_measurements)

    # for each measurement, calculate local volume fraction and 
    # update sampled voxels array
    for i in 1:nr_measurements

        # perform a single measurement on local volume fraction 
        # and update the sampled voxels array
        local_volume_fract, sampled_voxels_array = 
            get_local_volume_fract_and_update_sampled_voxels!(
                nr_dimensions_data,
                edge_length_window,
                data_binary,
    ...
    nr_independent_samples = get_nr_independent_samples(
        nr_dimensions_data, 
        edge_length_window, 
        sampled_voxels_array)
...
"""
function get_window_edge_length_vec(
    ...
    # determine by which even number the window edge length should be 
    # incremented at each step depending on the size of the total edge length
    # and the desired number of window sizes
    window_edge_length_increment = maximum([2, 
        Int64( 2*ceil( mean_edge_length_data/(4*nr_window_sizes) ) ) ] )  

    # create vector with measuring window edge lengths according to 
    # determined increment
    window_edge_length_vec = 
        collect(3:window_edge_length_increment:max_window_edge_length)
...
"""
function get_local_volume_fract_variance(
        volume_fract_tot::Float64,
        local_volume_fract_vec::Vector{Float64} )

    # calculate local volume fraction variance
    local_volume_fract_variance = Statistics.mean( 
        (local_volume_fract_vec .- volume_fract_tot ) .^2 )
...
"""
determine uncertainty on the local volume fraction using the equation given 
in this discussion:
https://math.stackexchange.com/questions/72975/variance-of-sample-variance
"""
function get_uncertainty_local_volume_fract_variance(
    nr_measurements,
    volume_fract_tot::Float64,
    local_volume_fract_vec::Vector{Float64},
    ...
    fourth_central_moment = Statistics.mean( 
        (local_volume_fract_vec .- volume_fract_tot ) .^4 )

    # calculate uncertainty on the local volume fraction
    uncertainty_local_volume_fract_variance = sqrt( 
        fourth_central_moment / nr_measurements
...
function get_local_volume_fract_variance_by_window_vec(
    ...
    window_edge_length_vec = get_window_edge_length_vec(
        structure_dict["mean_edge_length_data"]; nr_window_sizes )

    # create vector where for each window size the 
    # local volume fraction variance and its uncertainty will be stored
    local_volume_fract_variance_vec = zeros(
        Measurements.Measurement, length(window_edge_length_vec))

    # for each measuring window size get 
    # local volume fraction variance and its uncertainty
    for i in eachindex(window_edge_length_vec)

        # get vector of local volume fractions and the 
        # number of independent samples
        local_volume_fract_vec, nr_independent_samples = 
            get_local_volume_fract_vec(
        ...
        local_volume_fract_variance = get_local_volume_fract_variance(
            structure_dict["volume_fract_tot"],
            local_volume_fract_vec )

        # determine uncertainty on variance of local volume fraction using 
        # the number of independent samples
        uncertainty_local_volume_fract_variance = 
            get_uncertainty_local_volume_fract_variance(
        ...
        local_volume_fract_variance_vec[i] = Measurements.measurement(
            local_volume_fract_variance, 
            uncertainty_local_volume_fract_variance)
    ...
    local_volume_fract_variance_dict = Dict(
        "window_edge_length_vec" => window_edge_length_vec,
    ...
    local_volume_fract_variance_dict = modify_keys_in_dict(
        local_volume_fract_variance_dict, voxel_edge_length, label)
