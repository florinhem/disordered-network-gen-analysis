...
"""
Get 2 random coordinates that lie inside the structure at a given distance to each
other
"""
function get_random_coordinates(sampling_distance,
                                size_data::Tuple)
    ...
    # get second random coordinate at distance r from x1 by getting a random direction
    # according to the equations 1 and 2 in this website
    # https://mathworld.wolfram.com/SpherePointPicking.html

    theta = 2*pi*rand()
    phi = acos( 2 * rand() - 1 )

    x2 = Int.( round.( x1 .+ sampling_distance .* ( cos(phi) *sin(theta),
                                                    sin(phi)*sin(theta),
                                                    cos(theta) )  ) ) 
...
function get_autocovariance_fct_isotrope(sampling_distance,
    ...
    two_point_prob_fct_summand_vec = Vector{Float64}(undef, nr_measurements_per_distance)
...
function get_nr_sampling_distances(mean_edge_length_data::Int64)
    ...
    # get the length of the vector with entries 0:1:Int(round(mean_edge_length_data/2))
...
"""
get a vector of sampling distances at which the 2 point prob. function and then
the autocovariance function are measured. Based on this vector the discrete Fourier
transformation will be determined to calculate the spectral density
"""
function get_sampling_distance_vec(mean_edge_length_data::Int64; 
                nr_sampling_distances::Int64 = get_nr_sampling_distances(mean_edge_length_data) )

    # get vector of sampling distances
    sampling_distance_vec = collect(LinRange(0, 
...
"""
determine autocovariance function as a function of sampling distance for isotrope
3d binary media
"""
function get_autocovariance_fct_isotrope_by_sampling_distance_vec(structure_dict::Dict;
    nr_sampling_distances::Int64 = get_nr_sampling_distances(structure_dict["mean_edge_length_data"] ),
    nr_measurements_per_distance::Int64 = 10000,
    ...
    autocovariance_fct_vec = Vector{Measurements.Measurement}(undef, nr_sampling_distances)
    ...
    for i in eachindex(sampling_distance_vec)
        ...
        autocovariance_fct_vec[i] = get_autocovariance_fct_isotrope(sampling_distance_vec[i],
    ...
    autocovariance_fct_dict = Dict("sampling_distance_vec" => sampling_distance_vec,
    ...
    if save_result
        ...
        GU.save_dict_to_h5(copy(autocovariance_fct_dict), save_path*"_autocovariance_fct.h5")
...
3d media
"""
function get_spectral_density_isotrope(autocovariance_fct_vec::Vector,
                                        wavenumber::Float64;
    ...
    # get sampling distance step length (inverval between adjacent sampling distances)
    sampling_distance_step_length = sampling_distance_vec[2] - sampling_distance_vec[1]
    ...
    # from sampling distances and corresponding autocovariance_fcts, calculate spectral
    # density, which is the Fourier transform of the autocovariance fct
...
function get_wavenumber_vec(sampling_distance_vec::Vector)
    ...
    fundamental_wavenumber = 2*pi/(sampling_distance_step_length*nr_sampling_distances)

    # determine sampled wavenumbers vec
    wavenumber_vec = collect(1:floor(nr_sampling_distances/2)) .* fundamental_wavenumber
...
media.
The argument sampling_distance_cutoff is the sampling distance in units of nm, up to which
the autocovariance function will be Fourier transformed to yield the spectral density.
This cutoff is necessary, because above a certain threshold, the autocovariance function
is dominated by noise, which falsifies the Fourier transform
"""
function get_spectral_density_isotrope_by_wavenumber_vec(structure_dict::Dict;
    nr_sampling_distances::Int64 = get_nr_sampling_distances(structure_dict["mean_edge_length_data"]),
    nr_measurements_per_distance::Int64 = 10000,
    sampling_distance_vec = get_sampling_distance_vec(structure_dict["mean_edge_length_data"]; 
                                nr_sampling_distances = nr_sampling_distances),
    autocovariance_fct_dict = get_autocovariance_fct_isotrope_by_sampling_distance_vec(structure_dict;
                                nr_sampling_distances = nr_sampling_distances,
                                nr_measurements_per_distance = nr_measurements_per_distance,
    ...
    wavenumber_vec = get_wavenumber_vec(autocovariance_fct_dict["sampling_distance_vec"] )
    ...
    # uncertainty will be stored
    spectral_density_vec = Vector{Measurements.Measurement}(undef, nr_wavenumbers)
    ...
    for i in eachindex(wavenumber_vec)
        ...
        spectral_density_vec[i] = get_spectral_density_isotrope(
                                    ...
                                    wavenumber_vec[i];
                                    sampling_distance_vec=autocovariance_fct_dict["sampling_distance_vec"],
    ...
    # save results if desired
    if save_result
        GU.save_dict_to_h5(copy(spectral_density_dict), save_path*"_spectral_density.h5")
