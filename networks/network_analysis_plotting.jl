...
"""
function plot_structure_factor_heatmap(structure_factor_dict::Dict,
    save_path::String;
    ...
    if wavevector_component_to_fix == 1
        ...
        structure_factor_2d_array = structure_factor_dict["structure_factor_array"][wavevector_fixed_index,:,:] 
        
        #set labels and title for the plot
        xlabel = Latex.L"k_y / d^{-1}" 
        ylabel = Latex.L"k_z /  d^{-1}"
        title = (title
        *Format.format(Latex.L", k_x = {:.2f}", 
                    structure_factor_dict["wavenumber_vec_vec"][1][wavevector_fixed_index] )*" "*Latex.L" d^{-1}")
    ...
    elseif wavevector_component_to_fix == 2
        ...
        ylabel = Latex.L"k_z /  d^{-1}"
        title = (title
        *Format.format(Latex.L", k_y = {:.2f}", 
                    structure_factor_dict["wavenumber_vec_vec"][2][wavevector_fixed_index] )*" "*Latex.L" d^{-1}")
    ...
    elseif wavevector_component_to_fix == 3
        ...
        title = (title
        *Format.format(Latex.L", k_z = {:.2f} ", 
                    structure_factor_dict["wavenumber_vec_vec"][3][wavevector_fixed_index] )*" "*Latex.L" d^{-1}")
    ...
    if x_y_lims !== nothing
        Plots.heatmap!(structure_factor_plot, xlims = x_y_lims, ylims = x_y_lims, size = 500 .* (1 , 1 ))
    end

    #if specified by the argument, save the plot
    if  save_plot
        Plots.savefig(structure_factor_plot, save_path*"_structure_factor_2d.png")
...
function plot_binary_structure(data_binary::Array{Bool, 3})
    ...
    scene = GLMakie.meshscatter(coordinates_matrix[1,:],
                        ...
                        marker=GLMakie.FRect3D(GLMakie.Point3f0(-0.5, -0.5, -0.5), GLMakie.Vec3f0(1, 1, 1)), markersize = 1)
